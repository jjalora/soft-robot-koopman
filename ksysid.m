classdef ksysid
    ...
    properties
        ...
        includeConst; % include constant term
...
%             PxTPx = U * S * V';
        ...
        function [ out , obj ] = get_model( obj , koopData )
            ...
            % (JOHN) Modification 2: Enable specification of different
...
%             [ ysim , tsim , zsim ] = lsim(model.sys, ureal , treal , z0); % Don't use lsim. Might be doing weird stuff
            ...
            zsim = zeros( size( zreal * V ) ); % preallocate (TODO: project to modes)
            ysim(1,:) = yreal(1,:); % initialize
            zsim(1,:) = W * z0;        % initialize (TODO: project to modes)
            % TODO: Projected system matrices
            for j = 1 : length(treal)-1
                zsim(j+1,:) = ( W * model.A * V * zsim(j,:)' + W * model.B * usim(j,:)' )';
                ysim(j+1,:) = ( model.C * V * zsim(j+1,:)' )';
            end
